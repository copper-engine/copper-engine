			...
			try {
				new RetryingTransaction(commands.iterator().next().dataSource) {
					...
					protected void execute() throws Exception {
						final PreparedStatement stmtDelQueue = getConnection().prepareStatement("DELETE FROM COP_QUEUE WHERE WORKFLOW_INSTANCE_ID=? AND PPOOL_ID=? AND PRIORITY=?");
						final PreparedStatement stmtUpdateState = getConnection().prepareStatement("UPDATE COP_WORKFLOW_INSTANCE SET STATE=? WHERE ID=?");
						final PreparedStatement stmtInsertError = getConnection().prepareStatement("INSERT INTO COP_WORKFLOW_INSTANCE_ERROR (WORKFLOW_INSTANCE_ID, EXCEPTION, ERROR_TS) VALUES (?,?,NOW())");
