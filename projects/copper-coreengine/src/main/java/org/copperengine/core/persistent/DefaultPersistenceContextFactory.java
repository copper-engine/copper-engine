...
public final class DefaultPersistenceContextFactory implements PersistenceContextFactory<PersistenceContext> {
     ...
     * 
     * @param configuration
     *            The persister factory configuration
     * @param con
     *            The connection that this persistence context factory will flush to. See {@link #flush()}.
     */
                    ...
                    for (Object obj : sharedRessources.getWorkers()) {
                        ...
                        switch (worker.getOperationType()) {
                            case INSERT:
                                break;
                            case UPDATE:
                                position += 1 * numberOfPersisterFactories;
                                break;
                            case SELECT:
                                position += 2 * numberOfPersisterFactories;
                                break;
                            case DELETE:
                                position = 4 * numberOfPersisterFactories - position;
                                break;
                            default:
                                throw new RuntimeException("Unsupported operationType: " + worker.getOperationType());
                        }
    ...
    public void flush() throws SQLException {
        ...
        DefaultPersistenceWorker worker : orderedWorkers.values()) {
