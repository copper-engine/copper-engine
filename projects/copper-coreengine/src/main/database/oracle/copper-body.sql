	...
	LOOP
		...
		IF l_WORKFLOW_INSTANCE_ID.count <> 0 THEN
			FORALL i IN 1..l_WORKFLOW_INSTANCE_ID.count
				INSERT INTO COP_QUEUE (PPOOL_ID, PRIORITY, LAST_MOD_TS, wfi_rowid) (SELECT PPOOL_ID, PRIORITY, SYSTIMESTAMP, ROWID FROM COP_WORKFLOW_INSTANCE WHERE ID=l_WORKFLOW_INSTANCE_ID(i) AND STATE=5);
			FORALL i IN 1..l_WORKFLOW_INSTANCE_ID.count
