...
IS
  CURSOR cSelect IS SELECT rowidtochar(rowid) FROM COP_WORKFLOW_INSTANCE WHERE ID=i_WORKFLOW_INSTANCE_ID AND (STATE=c_STATE_ERROR OR STATE=c_STATE_INVALID) FOR UPDATE;
  WF_ROWID VARCHAR2(128);
BEGIN
  OPEN cSelect;
  FETCH cSelect INTO WF_ROWID;
  o_ROWCOUNT := cSelect%ROWCOUNT;
  CLOSE cSelect;
  IF o_ROWCOUNT > 0 THEN
     DELETE FROM COP_RESPONSE WHERE CORRELATION_ID IN (SELECT CORRELATION_ID FROM COP_WAIT WHERE WORKFLOW_INSTANCE_ID=i_WORKFLOW_INSTANCE_ID);
     DELETE FROM COP_WAIT WHERE WORKFLOW_INSTANCE_ID=i_WORKFLOW_INSTANCE_ID;
     DELETE FROM COP_WORKFLOW_INSTANCE_ERROR WHERE WORKFLOW_INSTANCE_ID=i_WORKFLOW_INSTANCE_ID;
     DELETE FROM COP_WORKFLOW_INSTANCE WHERE ID=i_WORKFLOW_INSTANCE_ID;
  END IF;
END;
