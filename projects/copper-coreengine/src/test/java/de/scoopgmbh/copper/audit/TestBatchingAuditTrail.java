...
public class TestBatchingAuditTrail {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i=0; i<100; i++) {
			auditTrail.asynchLog(2, new Date(), "conv12345678901234567890123456789012", "ctx", "proc12345678901234567890123456789012", "corr12345678901234567890123456789012", "testMessage");
		}
		Thread.sleep(2000);
	...
	private static void test(final String msg, AuditTrail auditTrail, final int max) throws InterruptedException {
		...
		for (int i=0; i<max; i++) {
			auditTrail.asynchLog(2, new Date(), "conv12345678901234567890123456789012", "ctx", "proc12345678901234567890123456789012", "corr12345678901234567890123456789012", msg, cb);
		}
		while (x.get() != max) {
			Thread.sleep(5);
		}
