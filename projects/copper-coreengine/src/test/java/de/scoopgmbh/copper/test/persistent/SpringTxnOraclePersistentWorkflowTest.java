...
public class SpringTxnOraclePersistentWorkflowTest extends PersistentWorkflowTest {
	
	private static final String DS_CONTEXT = "SpringTxnPersistentWorkflowTest/oracle-unittest-context.xml";
	...
	static {
		final ConfigurableApplicationContext context = new SpringTxnOraclePersistentWorkflowTest().createContext(DS_CONTEXT);
		try {
	...
	public void testSpringTxnUnitTestWorkflow() throws Exception {
		final ConfigurableApplicationContext context = createContext(DS_CONTEXT);
		cleanDB(context.getBean(DataSource.class));
